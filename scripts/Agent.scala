...
trait Agent extends NativeTradingAgent {
  ...
  def getUlProjectedPrice(ulInst: InstrumentDescriptor, direction: Direction): Either[Error, BigDecimal] =
    getProjectedPrice(ulInst)
      .map(BigDecimal(_))
      .map(Algo.getPriceAfterTicks(if(direction == Direction.BUY) true else false, _))
    .match {
  ...
  def getAbsoluteResidual(pointValue: Double): Option[BigDecimal] =  source[RiskPositionByULContainer].get(portfolioId, ulId, true)
    .latest.map(p => BigDecimal(p.getTotalPosition.getDeltaCashUlCurr/p.getTotalPosition.getUlSpot/pointValue))
  ...
  def preProcess[F[_]: Monad]: EitherT[F, guardian.Error, Order] =
    for {
      ...
      _ <- EitherT.rightT(Either.cond(order.getQuantityL > 0, (), Error.StateError("Pre-process order qty cannot be negative")))
    } yield order

  def sendOrderAction(act: OrderAction): Order = act match {
